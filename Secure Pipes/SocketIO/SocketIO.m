 ...
 withConnectionTimeout:(NSTimeInterval)connectionTimeout
{
    if (!_isConnected && !_isConnecting) {
        ...
        NSString *port = _port ? [NSString stringWithFormat:@":%d", (ushort)_port ] : @"";
...
- (void) onData:(NSString *)data
{
    ...
    switch(_version)
    {
        ...
        case V10x:
        {
            ...
            SocketIOPacket *packet = [[SocketIOPacketV10x alloc] initWithTypeIndex:(ushort)control];
                    ...
                    switch (control) {
                        ...
                        case 3:
                        {
                            ...
                            if (ack > 0) {
                                ...
                                if (argsStr && ![argsStr isEqualToString:@""]) {
                                    argsData = [SocketIOJSONSerialization objectFromJSONData:[argsStr dataUsingEncoding:NSUTF8StringEncoding] error:&error];
                                    // either send complete response or only the first arg to callback
                                    if (!_returnAllDataFromAck && [argsData count] > 0) {
                                        argsData = [argsData objectAtIndex:0];
                                    }
